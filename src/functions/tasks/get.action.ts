...
export const getTask: ValidatedEventAPIGatewayProxyEvent<typeof schema> = async (event, context) => {
  context.callbackWaitsForEmptyEventLoop = false;

  const client = await clientPromise;
  const dbConnection = client.db();
  const idQuery = { _id: new ObjectId( event.pathParameters.id )};
  const result = await dbConnection.collection('tasks').findOne(idQuery);
